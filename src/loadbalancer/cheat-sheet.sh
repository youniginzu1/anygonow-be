export $(grep -v '^#' .env | xargs) && envsubst < values.tmp.yaml > values.yaml
...
helm upgrade -n ${NAMESPACE} ${project} .
kubectl -n ${NAMESPACE} get pods
kubectl -n ${NAMESPACE} get svc
kubectl -n ${NAMESPACE} get deployment
kubectl -n ${NAMESPACE} describe deployment ${project}
helm uninstall -n ${NAMESPACE} ${project}
kubectl -n ${NAMESPACE} port-forward authservice-9586d6785-9f78g 50051:50051