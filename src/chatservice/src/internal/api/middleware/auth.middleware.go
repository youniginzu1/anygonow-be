...
func (s *MiddlewareV1) CheckAuth(g *gin.Context) {
	if unleash.IsEnabled("chatservice.auth.by-pass") {
		s.Logger.Warn("by pass check auth ========================")
		s.byPassCheckAuth(g)
	} else {
		s.checkAuth(g)
	}
...
func (s *MiddlewareV1) checkAuth(g *gin.Context) {
	ctx, span := otel.GetTracerProvider().Tracer(c.SERVICE_NAME).Start(g.Request.Context(), lib.GetFunctionName(s.checkAuth))
	defer span.End()
